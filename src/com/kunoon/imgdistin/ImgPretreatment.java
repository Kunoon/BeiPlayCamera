...
public class ImgPretreatment {
	...
	private static Bitmap binarization(int T) {
		...
		for (int i = 0; i < imgHeight; i++) {
			for (int j = 0; j < imgWidth; j++) {
				...
				if (gray < T) {
					imgPixels[i * imgWidth + j] = Color.rgb(0, 0, 0);	// 小于阈值设为白色
				} else {
					imgPixels[i * imgWidth + j] = Color.rgb(255, 255, 255);	// 大于阈值设为黑色
				}
			}
		}
		Bitmap result = Bitmap.createBitmap(imgWidth, imgHeight, Config.RGB_565);
	...
	private static Bitmap selectBinarization(int[] T) {
		for (int i = 0; i < imgHeight; i++) {
			for (int j = 0; j < imgWidth; j++) {
				int gray = imgPixels[i * imgWidth + j];
				if (gray < T[0] && gray < T[1] || gray < T[0] && gray < T[2] || gray < T[1] && gray < T[2]) {
